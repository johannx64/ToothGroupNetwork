...
os.environ["OPEN3D_CPU_RENDERING"] = "true"
os.environ["PYOPENGL_PLATFORM"] = "egl"  # or "osmesa" depending on your system
...
def print_3d(mesh, save_as_image=True, image_path="output.png"):
    try:
        ...
        # Remove axes and other visualization elements
        ...
        vis.destroy_window()
        print(f"Rendered view saved as {image_path}")
        
        # Additionally save the mesh/point cloud as PLY for backup
        ply_path = image_path.replace('.png', '.ply')
        if isinstance(mesh, o3d.geometry.PointCloud):
            o3d.io.write_point_cloud(ply_path, mesh)
        else:
            o3d.io.write_triangle_mesh(ply_path, mesh)
        print(f"Geometry saved as {ply_path}")
        
    except Exception as e:
        print(f"Failed to render: {str(e)}")
        # Fallback: save just the geometry
        try:
            ply_path = image_path.replace('.png', '.ply')
            if isinstance(mesh, o3d.geometry.PointCloud):
                o3d.io.write_point_cloud(ply_path, mesh)
                ...
                o3d.io.write_triangle_mesh(ply_path, mesh)
            print(f"Saved geometry as {ply_path} (rendering failed)")
        except:
            print("Failed to save geometry")
...
# Optional: Set this flag to ensure no GUI/OpenGL operations are attempted
